...
Latest update:
- 2026-02-13: Fixed missing observability for persistence-summary callback flow in `RoomCommandHandlerTs`: when internal monologue summaries are generated, TS now logs a DEBUG event (`Persisting internal monologue summary`) with arc/chars/preview before writing `[internal monologue] ...` to history; added regression assertion in `ts/tests/command-handler.test.ts`.
- 2026-02-13: Adjusted direct-command lifecycle verbosity in `RoomCommandHandlerTs` so `Handling direct command` and `Persisting direct command response` log at DEBUG (not INFO), reducing stdout noise while retaining detail in per-message/system log files; updated `ts/tests/command-handler.test.ts` expectations.
- 2026-02-13: Adjusted TS agent tool execution logging for Python-style parity in `MuaddibAgentRunner`: INFO now emits a single summary line per tool (`Tool <name> executed: <preview>...` / `Tool <name> failed: <preview>...`), while detailed tool payloads/errors stay in DEBUG (`... details:`) for per-message/system log files. Updated `ts/tests/muaddib-agent-runner.test.ts` expectations accordingly.
- 2026-02-13: Fixed TS `anthropic:claude-opus-4-6` model-resolution parity by upgrading `@mariozechner/pi-ai` + `@mariozechner/pi-agent-core` to `0.52.10` (lockfile refresh), and aligned TS CLI message-mode observability with runtime logging (structured file logs under `$MUADDIB_HOME/logs` + per-message context routing in `ts/src/cli/message-mode.ts` + reduced stdout tool-result verbosity in `MuaddibAgentRunner` with debug-level detail persistence). Added/updated regression coverage in `ts/tests/cli-message-mode.test.ts` and `ts/tests/muaddib-agent-runner.test.ts`.
...
- 2026-02-13: Restored artifact viewer parity in TS by externalizing the viewer template to `ts/src/agent/tools/artifact-viewer.html` and wiring `core-executors` to load/install that file (instead of the previous minimal inline HTML fallback), with updated executor coverage in `ts/tests/core-executors.test.ts`.
